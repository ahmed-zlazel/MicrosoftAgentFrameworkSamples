...
builder.Services.AddChatClient(azureOpenAIClient.GetChatClient(configuration.ChatDeploymentName)
    .AsIChatClient()); //You need to register a chat client for the dummy agents to use
builder.Services.AddOpenAIResponses();
builder.Services.AddOpenAIConversations();

// Register "dummy" Agent
builder.AddAIAgent("Dummy Weather", "You are weather expert ")
...
AIAgent myAgent = azureOpenAIClient
    ...
    .CreateAIAgent(name: realAgentName,
...
{
    IEnumerable<AIAgent> agentsForWorkflow = new List<IHostedAgentBuilder>() { arabicTranslator, frenchTranslator }
...
{
    IEnumerable<AIAgent> agentsForWorkflow = new List<IHostedAgentBuilder>() { arabicTranslator, frenchTranslator }.Select(ab => sp.GetRequiredKeyedService<AIAgent>(ab.Name));
